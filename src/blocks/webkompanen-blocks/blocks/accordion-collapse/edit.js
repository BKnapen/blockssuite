...
const sectionEdit = (props) => {
	...
	const blockid = attributes.blockid ? attributes.blockid : ''
	const blockID = blockid == '' ? null : blockid

	const parentid = attributes.parentid ? attributes.parentid : ''
	const parentID = parentid == '' ? null : parentid
	...
	let blockClasses = 'accordion-collapse collapse';

	blockClasses += classes != null && classes != '' ? ' '+classes : ''
	blockClasses += attributes.collapse ? ' show' : ''
	...
	return(
		<>		
			<Fragment>		
				<InspectorControls>
					<InputControl
						label={__('Accordion ID', 'webkompanen')}
						labelPosition="top"
						value={ parentID }
						type="text"
					...
					<ToggleControl
						...
						help={ attributes.collapse ? __('Yes.', 'webkompanen') : __('No.', 'webkompanen') }
			...
			
			{
				bgimg && (
  					<div 
						...
						data-bs-parent={ attributes.parentid ? '#'+attributes.parentid : null }
					>
