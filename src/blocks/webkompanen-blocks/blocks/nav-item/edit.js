...
const sectionEdit = (props) => {
	...
	const hasInnerBlocksArr = useSelect( ( select ) =>
	...
	let dropdown = 0;
	if(hasInnerBlocks){
		hasInnerBlocksArr.map(
			(arr, index) =>(
				arr.name === 'webkompanen-blocks/dropdown-menu' ? dropdown++  : ''
			)
		)
	}
	console.log('hasInnerBlocksArr')
	console.log(hasInnerBlocksArr)
	console.log(dropdown)

	dropdown = dropdown > 0 ? true : false

	if(dropdown !== attributes.dropdown){
		setAttributes(
			{dropdown:dropdown}
		)
	}
	console.log('attributes.dropdown')
	console.log(attributes.dropdown)
	console.log(dropdown)
	...
	blockClasses += dropdown != null && dropdown != '' ? ' dropdown' : ''
